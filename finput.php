...
if(!empty($_POST['tgtdate']) && !empty($_POST['tgtcategory']) && !empty($_POST['tgtitem']))
{
    $rcvDate = $_POST['tgtdate'];
    $rcvCategory = $_POST['tgtcategory'];
    $rcvItem = $_POST['tgtitem'];
    $rcvPrice = $_POST['tgtprice'];
    $rcvQuantity = $_POST['tgtquantity'];
    $rcvCalory = $_POST['tgtcalory'];

   $food = new Food();
   $food->setDate($rcvDate);
   $food->setCategory($rcvCategory);
   $food->setItem($rcvItem);
   $food->setPrice($rcvPrice);
   $food->setQuantity($rcvQuantity);
   $food->setCalorie($rcvCalory);
   $food->save();
...
<div class="container mt-xl-5 mx-auto align-items-center">
    ...
    <form id="einput" action="finput.php" method="post">
        ...
        <div class="form-row form-inline mt-2 mb-3">
          ...
          <input type="text" class="form-control col-md-9 h4" id="price" placeholder="支出額" name="tgtprice">
        </div>
        <!--/価格-->

        <!--数量-->
        <div class="form-row form-inline mt-2 mb-3">
          <label for="quantity" class="col-md-3 h3">数量</label>
          <input type="text" class="form-control col-md-9 h4" id="quantity" placeholder="購入個数" name="tgtquantity">
        </div>
        <!--/数量-->

        <!--熱量-->
        <div class="form-row form-inline mt-2 mb-3">
          <label for="calory" class="col-md-3 h3">熱量</label>
          <input type="text" class="form-control col-md-9 h4" id="calory" placeholder="個当りカロリー" name="tgtcalory">
        </div>
        <!--/熱量-->
